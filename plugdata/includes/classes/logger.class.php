...
*
* $sirv_gbl_sirv_logger = new SirvLogger($path_to_plugin_dir, $excluded_path, $log_filename = null, $dir_to_logs = null, $is_append_logs_to_file = true)
*
...
*
* $sirv_gbl_sirv_logger->debug($msg, $var_name ='')->print($is_die=false);
* $sirv_gbl_sirv_logger->error($msg, $var_name = '')->write();
*
*
* Possible to use chaining style. Possible chaining methods: filename, mode, dir, dir_path, filepath
*
* $sirv_gbl_sirv_logger->debug($array_var, '$array_var')->filename('test.log')->write();
* $sirv_gbl_sirv_logger->debug($array_var, '$array_var')->filename('test.log')->mode('a+')->write();
* $sirv_gbl_sirv_logger->info($object_var, '$object_var')->dir('another_logs')->filename('info.log')->write();
* $sirv_gbl_sirv_logger->warning('Some text')->dir_path('/var/www/site/wp-content/plugins/plugin/somelogs')->write();
* $sirv_gbl_sirv_logger->warning($bool_var)->filepath('/var/www/site/wp-content/plugins/plugin/somelogs/log_name.log')->write();
*
...
* Performance methods
*
* $sirv_gbl_sirv_logger->time_start($timer_name);
* //some code
* $sirv_gbl_sirv_logger->time_chunk($timer_name, $chunk_name = null);
* //some code
* $sirv_gbl_sirv_logger->time_end($timer_name, $decimals = 6);
*
